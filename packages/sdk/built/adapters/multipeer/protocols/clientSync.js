...
class ClientSync extends Protocols.Protocol {
    constructor(client) {
         ...
         */
        this['stage:load-assets'] = async () => {
            // Send all cached load-assets messages.
            await Promise.all(this.client.session.assets.map(message => this.sendAndExpectResponse(message)));
            // Send all cached asset-update messages.
            this.client.session.assetUpdates.map(payload => this.sendMessage({ payload }));
        };
        /**
         * @hidden
         ...
         */
        this['stage:active-sound-instances'] = () => {
            // Send all cached set-behavior messages.
         ...
         */
        this['stage:sync-animations'] = async () => {
    ...
    sendMessage(message, promise) {
        ...
        switch (handling) {
            ...
            case 'error': {
                ...
                log_1.log.error('network', `[ERROR] ${this.name}: Invalid message for send during synchronization stage: ${message.payload.type}. In progress: ${this.inProgressStages.join(',')}. Complete: ${this.completedStages.join(',')}.`);
            }
        }
    }
    /** @override */
    ...
    createActorInterpolations(actor) {
        for (let activeInterpolation of actor.activeInterpolations || []) {
            ...
            activeInterpolation = Object.assign({}, activeInterpolation, { enabled: false });
